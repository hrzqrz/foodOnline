        ...
        if self.pk is not None:
            ...
            if orig.is_approved != self.is_approved:
                ...
                if self.is_approved == True:
                    #send notification emeial
                    mail_subject = 'Congratulations! Your restaurant has been approved.'
                    send_notification(mail_subject, mail_template, context)
                else:
                    #send notification email
                    mail_subject = 'We are sorry! You are not eligible for publishing your food menu on our marketpalce.'
                    send_notification(mail_subject, mail_template, context)
...
HOUR_OF_DAY_24 = t = [(time(h, m).strftime('%I:%M %p'), time(h, m).strftime('%I:%M %p')) for h in range(0, 24) for m in (0, 30)]
